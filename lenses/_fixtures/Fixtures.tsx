...
export const FakeDashboard: React.FC<{ variant?: 'A' | 'B' }> = ({ variant = 'A' }) => (
  <div style={{ width: 1920, height: 1080, background: G.bg, display: 'flex' }}>
    <div style={{ width: 220, background: G.nav, padding: '28px 22px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 18 }}>
...
export const TitleBlock: React.FC<{ text: string; size?: number }> = ({ text, size = 88 }) => (
  <div style={{ fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 800, fontSize: size, color: G.ink, letterSpacing: -1, textShadow: '2px 2px 8px rgba(211,146,60,0.28)' }}>
    {text}
  </div>
);
